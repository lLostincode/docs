...
description: 'Master prompt engineering in Artaios with dynamic context control, few-shot learning, and history management'
---
...
prompt_config:
  ...
  include_internal_messages: bool    # Include internal agent outputs (default: true)
...
<CardGroup cols={2}>
  <Card title="Custom Variables" icon="sliders">
...
|---------|-------------|---------|
| `{{ agent_name.output.field }}` | Access specific field | `{{ analyzer.output.key_topics }}` |
| `{{ agent_name.output }}` | Full structured output (JSON) | `{{ analyzer.output }}` |
| `{{ agent_name.output.field.subfield }}` | Nested field access | `{{ analyzer.output.customer.name }}` |
...
agents:
  ...
  - name: response_generator
    ...
    prompt_config:
      system_prompt: |
        ## Input Analysis
        **Topics**: {{ input_analyzer.output.key_topics }}
        **Sentiment**: {{ input_analyzer.output.sentiment }}
        **Complexity**: {{ input_analyzer.output.complexity }}
        **Entities**: {{ input_analyzer.output.entities }}
...
| Pattern | Description |
|---------|-------------|
| `{{ variables.name }}` | Value of a custom variable |
...
<Info>
Variables are the bridge between structured output and prompt substitution. Use `variable_assignments` on agents to populate variables from structured output, then reference them with `{{ variables.name }}` in downstream agents. See [Variables System](/documentation/advanced/variables-system) for details.
</Info>
  ...
  - name: report_generator
    ...
    prompt_config:
      system_prompt: |
        ...
        **Trends Identified**: {{ data_analyzer.output.trends }}
        **Anomalies Detected**: {{ data_analyzer.output.anomalies }}
        **Recommendations**: {{ data_analyzer.output.recommendations }}
...
prompt_config:
  ...
  include_internal_messages: true  # See all agent outputs
```

**Behavior**: Both user-facing messages and internal agent outputs appear in conversation history.

**Use Case**: Agents that need full awareness of the multi-agent workflow state.

### Exclude Internal Messages

```yaml
prompt_config:
  system_prompt: You are a customer-facing assistant
  include_internal_messages: false  # Only see user-visible messages
```

**Behavior**: Only messages from agents with `show_output_to_user: true` and user messages appear in history. Internal agent outputs are filtered out.
...
| Variable | Description | Example |
|----------|-------------|---------|
| `{{ variables.name }}` | Custom variable value | `{{ variables.customer_name }}` |
...
| `{{ history }}` | Full user-visible conversation history | `{{ history }}` |
| `{{ history[N] }}` | Last N user-visible messages | `{{ history[5] }}` |
| `{{ full_history }}` | Complete execution trace (internal + tool calls) | `{{ full_history }}` |
| `{{ full_history[N] }}` | Last N messages (all types) | `{{ full_history[10] }}` |
| `{{ agent_name.output.field }}` | Structured output field | `{{ analyzer.output.topics }}` |
| `{{ agent_name.output }}` | Last agent message (non-structured) | `{{ researcher.output }}` |
...
| `include_internal_messages` | bool | `true` | Include internal agent outputs (`show_to_user=false`) in history |
| `few_shot_examples` | list | `[]` | Teaching examples (each with `input` and `output` keys) |

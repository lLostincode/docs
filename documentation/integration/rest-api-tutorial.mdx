...
Inject dynamic values into your agents using variables. Variables allow you to pass user IDs, context, queries, and other data that gets substituted into agent prompts.

<Info>
Learn more in the [Using Variables in REST API](/documentation/integration/variables-in-rest-api) guide.
</Info>

**Session-level example** (persist across requests):
...
**Request-level example** (fresh on every request):

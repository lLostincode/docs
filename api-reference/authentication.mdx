...
<Note>
Browser/dashboard sessions may also authenticate with a JWT Bearer token (`Authorization: Bearer <token>` header). This section covers `x-api-key`, the mechanism intended for external API consumers.
</Note>

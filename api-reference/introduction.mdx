...
<Steps>
  ...
  <Step title="Validate your agent configuration">
    Test your agent configuration before creating a session. The request body is a full system configuration (same shape as [YAML Configuration](/guides/yaml-configuration)), sent as-is - not wrapped in a `system_config` key:
  ...
  <Step title="Create a session and execute">
    Create a session with your configuration (again, the full config as the raw body) and start executing messages:
    ...
    # -> {"success": true, "session_id": "...", "config_hash": "..."}

...
<Note>
Files are private to the uploading user - each user can only see and manage their own files.
</Note>
...
<ResponseField name="file_size_formatted" type="string">
  Human-readable file size (e.g. `"2.0 MB"`)
</ResponseField>
...
<ResponseField name="file_url" type="string">
  Download URL, present only if the requesting user has access
</ResponseField>

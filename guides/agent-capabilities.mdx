...
Tools extend agent capabilities by providing access to external systems, APIs, and computational resources. `react_agent` is the only user-configurable agent type for tool calling (a `react_agent` with no `tools` simply runs as a plain conversational agent); `supervisor` agents also call tools internally for agent handoffs, but that's not user-configured via the `tools` field.
...
<CardGroup cols={3}>
  ...
  <Card title="google_search" icon="magnifying-glass">
    **Google Search with Gemini**

    Web search grounded through Gemini.
  </Card>

  <Card title="search_knowledge_bases" icon="database">
    **Search Knowledge Bases**

    Query an agent's connected knowledge bases as a tool call.
  </Card>
</CardGroup>

<Note>
This is a representative sample - the tool registry includes many more (image generation, text-to-speech, Wikipedia/ArXiv search, memory management, and others).
</Note>
...
Agents can access knowledge bases to retrieve domain-specific information and documents. `react_agent` supports knowledge base integration whether or not it has `tools` configured.

<Tip>
For comprehensive knowledge base configuration details, see the [Knowledge Base Setup](/guides/knowledge-base-setup) guide.
</Tip>
...
<Note>
`uknow` is also a supported `knowledge_base_type` (used for connected cloud-storage sources); see [Knowledge Base Setup](/guides/knowledge-base-setup) for the fully documented type. Additional vector database integrations will be supported in future releases.
</Note>
...
<Check>
Supervisors can coordinate `react_agent` workers with different `tools` configurations - some tool-enabled, some plain conversational - creating hybrid teams with diverse capabilities.
</Check>

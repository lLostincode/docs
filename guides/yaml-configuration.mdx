...
<ParamField path="persistent_state" type="boolean" default="true">
  Enable LangGraph checkpointing for state persistence across executions
</ParamField>

## Agent Configuration

Each agent in the `agents` list requires these fields:
...
**Agent-Level (Recommended)**: Configure tools for individual agents
...
**System-Level (Advanced)**: Define tools globally for reuse
...
### Supervisor Configuration (Supervisor Agents Only)

These fields are configured **inside the supervisor agent's configuration**.
...
For `workflow` architecture, define edges to connect agents:

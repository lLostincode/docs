...
<Tabs>
  ...
  <Tab title="Multimodal Message">
    ...
    <Tip>
    `source.type: "url"` (a public or presigned URL) is recommended over inline `base64` - it's far cheaper on tokens.
    </Tip>
  </Tab>
  
  <Tab title="File Upload">
    Upload the file first via `POST /api/v1/sessions/{session_id}/artifacts/`, then reference the returned artifact's presigned URL (or `artifact_id`, for URL refresh) in a `file` content block instead of inline base64:
    ...
    <Note>
    Inline `base64` file data is also accepted but is a legacy path - prefer uploading via the artifacts endpoint above for anything but small files.
    </Note>
  </Tab>
</Tabs>
...
A WebSocket endpoint is available at `wss://api-be.atthene.com/ws/amas` for authenticated, real-time streaming (the same transport the embedded chatbot uses internally). Full message-protocol documentation for direct WebSocket consumers is still in progress - if you need this for a non-embedded integration, please let us know via the feedback button.
